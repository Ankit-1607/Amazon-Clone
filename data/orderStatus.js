import { previousOrders, getPrevOrdersFromStorage } from "./previous-products.js";
...
export function saveOrderStatusToStorage() {
  localStorage.setItem('statusOfOrders', JSON.stringify(statusOfOrders));
}

export function getOrderStatusFromStorage() {
  statusOfOrders = JSON.parse(localStorage.getItem('statusOfOrders'));
}
...
export function getProductDetails(productID) {
  ...
  getOrderStatusFromStorage() // program will not reach here if previous order is null
  ...
  statusOfOrders.forEach((thisOrder) => {
    ...
    thisOrder.deliveryStatusesProds.forEach((thisProduct) => {
      if(thisProduct.productID == productID) {
        ...
        localStorage.setItem('thisProductTracks', JSON.stringify(thisProductTracks));
        return;
      }
    })
  })
